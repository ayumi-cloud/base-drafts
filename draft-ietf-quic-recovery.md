...
TCP reduces the congestion window to one packet when the SYN or SYN/ACK
are lost ({{?RFC5681}}).  QUIC treats loss of a packet containing handshake
data the same as other losses and reduces the congestion window.  When the
PTO timer expires during the handshake, one or two packets are sent and
the timer is doubled.
